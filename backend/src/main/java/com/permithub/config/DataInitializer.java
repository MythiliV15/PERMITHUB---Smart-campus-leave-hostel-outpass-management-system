...
public class DataInitializer implements ApplicationRunner {
    ...
    public void seedAll() {
        ...
        Student s1 = student("Arjun Sharma", "2k22it31@kiot.ac.in", "711521CS001", "CS3A01", 3, "A", cse, sem,
    ...
    private Student student(String name, String email, String regNo, String rollNo,
        ...
        // Idempotent: register number is unique — re-runs must update the same row,
        // not INSERT again (avoids duplicate key when user_id lookup misses stale rows).
        Optional<Student> existingByReg = studentRepo.findByRegisterNumber(regNo);
        String studentPhone = parentPhone;
        User u;
        Student s;
        if (existingByReg.isPresent()) {
            ...
            u = user(name, email, Role.STUDENT, dept, studentPhone);
            s = studentRepo.findByUserId(u.getId()).orElse(new Student());
            s.setUser(u);
        }

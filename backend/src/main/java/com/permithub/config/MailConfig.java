...
public class MailConfig {
    ...
    public JavaMailSender javaMailSender() {
        ...
        String pwd = password != null ? password.replaceAll("\\s+", "") : "";
        boolean configured = !user.isBlank()
                && !user.equals("your_gmail@gmail.com")
                && !pwd.isBlank()
                && !pwd.equals("your_16char_app_password");

        if (configured) {
            sender.setUsername(user);
            sender.setPassword(pwd);
            log.info("✅ Mail configured: {}:{} as {}", host, port, user);
        } else {

...
export default function MenteesPage() {
  ...
  const downloadDisplayed = () => {
    if (tab === 0) {
      if (mentees.length === 0) return
      downloadCsv(`my-mentees-students-${stamp()}.csv`, ['Name', 'Register No', 'Year', 'Section', 'Dept', 'Type', 'Parent Phone', 'Leave Balance'], mentees.map((s) => [
        s.name,
        s.registerNumber,
        s.year,
        s.section,
    ...
    if (tab === 1) {
      ...
      downloadCsv(`my-mentees-leave-requests-${stamp()}.csv`, [
        'Student', 'Register No', 'Status', 'Category', 'From', 'To', 'Days', 'Reason',
        'Mentor Status', 'Advisor Status', 'HOD Status', 'Emergency',
      ], leaves.map((l) => [
        l.studentName,
        l.registerNumber,
        l.status,
        ...
        l.advisorStatus || '',
        l.hodStatus || '',
        l.isEmergency ? 'Yes' : 'No',
      ]))
      return
    }
    if (tab === 2) {
      if (ods.length === 0) return
      downloadCsv(`my-mentees-od-requests-${stamp()}.csv`, [
        'Student', 'Register No', 'Status', 'Event Type', 'Event Name', 'From', 'To', 'Days', 'Location',
        'Mentor Status', 'Coordinator Status', 'Advisor Status', 'HOD Status',
      ], ods.map((o) => [
        o.studentName,
        o.registerNumber,
        o.status,
    ...
    downloadCsv(`my-mentees-outpass-requests-${stamp()}.csv`, [
      'Student', 'Register No', 'Status', 'Destination', 'Out', 'Return', 'Hours', 'Reason',
      'Mentor Status', 'Parent Status', 'Advisor Status', 'Warden Status', 'AO Status', 'Principal Status',
    ], outpasses.map((o) => [
      o.studentName,
      o.registerNumber,
      o.status,
      o.destination,
      o.outDatetime ? new Date(o.outDatetime).toISOString() : '',
      o.returnDatetime ? new Date(o.returnDatetime).toISOString() : '',
      o.durationHours ?? '',
      o.reason || '',
      o.mentorStatus || '',
      o.parentStatus || '',
  ...
  const downloadDisabled =
    (tab === 0 && mentees.length === 0) ||
    (tab === 1 && leaves.length === 0) ||
    (tab === 2 && ods.length === 0) ||
    (tab === 3 && outpasses.length === 0)
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        <button type="button" onClick={downloadDisplayed} disabled={downloadDisabled}
          className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-60 transition">
          Download
        </button>
      </div>

...
export default function MyClassPage() {
  ...
  const downloadDisplayed = () => {
    if (tab === 0) {
      ...
      downloadCsv(`my-class-students-${stamp()}.csv`, ['Name', 'Register No', 'Roll No', 'Email', 'Type', 'Blood Grp', 'Parent Phone'], students.map((s) => [
        s.name,
        s.registerNumber,
        s.rollNumber || '',
        s.email || '',
    ...
    if (tab === 1) {
      ...
      downloadCsv(`my-class-leave-requests-${stamp()}.csv`, [
        'Student', 'Register No', 'Status', 'Category', 'From', 'To', 'Days', 'Reason',
        'Mentor Status', 'Advisor Status', 'HOD Status',
      ], leaves.map((l) => [
        l.studentName,
        l.registerNumber,
        l.status,
    ...
    downloadCsv(`my-class-od-requests-${stamp()}.csv`, [
      'Student', 'Register No', 'Status', 'Event Type', 'Event Name', 'From', 'To', 'Days',
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button type="button" onClick={downloadDisplayed} disabled={downloadDisabled}
            className="flex items-center gap-1 bg-blue-600 text-white px-3 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-60 transition">
            Download
          </button>
        </div>
      </div>
